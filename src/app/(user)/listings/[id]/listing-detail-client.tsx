...
export default function ListingDetailClient({
  ...
  const id = listingId || (params?.id as string);
  ...
  useEffect(() => {
    ...
    const loadSimilar = async (l: Listing) => {
      try {
        const rows = await listingService.getListings({ type: l.type });
        if (!cancelled) setSimilarListings(rows.filter((x) => x.id !== id).slice(0, 4));
      } catch {
